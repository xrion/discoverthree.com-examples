...
function createRawShaderMaterial( camera ) {
  ...
  const rawShaderMaterial = new RawShaderMaterial( {
    ...
    uniforms: {
      modelMatrix: {
        ...
        // temp matrix, we'll replace it with mesh.matrixWorld
        value: new Matrix4(),
...
export default function createMaterials( camera ) {

  return {

    purple: createRawShaderMaterial( camera ),
